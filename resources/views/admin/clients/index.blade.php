            ...
            @forelse($users as $user)
                <tr>
                   <td>{{ $user->is_admin ? 'Админ' : 'Клиент' }}</td>
                    ...
                    <td>
                        <a href="" class="btn btn-warning rounded-circle" role="button">
                        ...
                        <form action="{{ route('admin.clients.destroy', $user->id) }}" method="POST" style="display:inline;">
                            <button type="submit" class="btn btn-danger rounded-circle" onclick="return confirm('Вы уверены, что хотите удалить пользователя?')">
